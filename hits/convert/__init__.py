...
def raw2csv(src, key, dest=None, verbose=0):
	now = datetime.now() if verbose >= 2 else None
	...
	#Retrieve and process raw data. Hardcoding to ignore the first 5 lines
	#and the 7th line (not important)
	raw = __process(__readLines(src), separator=",")
	...
	keys = __process(__readLines(key), separator=",")
	keyDict = {}
	for key in keys:
		keyDict[key[1]] = key[0]
	...
	return __write(dest, __clean(matrix, tagged, rows, cols), time=now, verbose=verbose, separator=",")
...
# @param tsv – the unprocessed tsv, [String]
# @return tsv - the processed tsv [[String]]
def __process(tsv, separator="\t"):
	tsv = [line.replace("\r\n", "").replace("\n", "").replace("\r", "") for line in tsv]
	tsv = map(lambda line: line.split(separator), tsv)
	for line in tsv:
		try:
			line.remove("")
		except:
...
def __write(dest, cleaned, time=None, verbose=0, separator="\t"):

	if dest is not None:
		with open(dest, "w") as destFile:
			for line in cleaned:
				destFile.write(separator.join(line) + "\n")
...
def tsv2hit(filePath): #requires subject id to be the first thing available

	tsv = filePath
	if isInstance(filePath, str):	#if what is provided is actually a string, treat it as a filepath. Else, treat it liek a loaded tsv
		tsv = __process(__readLines(filePath))

	hits = False

	properties = tsv.pop(0)
	if properties 
	for hit in tsv:
		subject
		for j in xrange(len(hit)):
			if j == 0: 		# if j is zero then we're dealing with the subject id.
